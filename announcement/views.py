...
def edit_announcement(request, a_id, a_slug):
    ...
    if request.method == 'GET':
        ...
        announcement_edit_form = forms.AnnouncementForm(data=request.POST, instance=announcement)
    ...
    context = {'announcement_edit_form': announcement_edit_form, 'announcement': announcement}
...
def draft_announcement(request):
    if request.user.is_student:
        raise PermissionDenied
    draft_announcement = Announcement.objects.filter(status='draft', author=request.user)
